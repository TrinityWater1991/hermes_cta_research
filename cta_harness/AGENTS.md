...
### 2026-05-31: AlphaFlow v2 波动率包络策略优化

**策略核心**: HLC/3均价 → 双EMA(20/50)基线 → ATR通道突破 → 仅做多
**优化历程**: 固定仓位(Sharpe 0.45) → 动态仓位(Sharpe 1.00)

#### 关键发现

1. **动态仓位是单标的策略的胜负手**
   - 公式: `size = max(1, int((capital / ATR) * position_multiplier))`
   - ATR高(波动大)→自动缩仓, ATR低(趋势稳)→加仓
   - 固定仓位 Sharpe 0.45 → 动态仓位 Sharpe 1.00 (+122%)
   - position_multiplier 用 0.001~0.01 范围扫描, 0.005 最优

2. **K线周期选择: 低频优于高频**
   - 1h: Sharpe 1.00, +160.6%, 520笔, 手续费26k
   - 30m: Sharpe 0.89, +69.6%, 490笔, 手续费20k (优化后)
   - 30m即使优化后也不及1h——高频带来更多噪音而非更多alpha

3. **加密单标的只做多天然占优**
   - BTC 2019-2026 长期牛市, 不做空避免了熊市假突破损失
   - 做空需要独立的信号逻辑, 不能简单对称反转

#### 工程教训

4. **回测进程内存: 每进程2.4GB, 禁止并行**
   - vnpy BacktestingEngine 加载353万条1m K线 → 2.4GB RSS
   - 并行跑2个以上回测会导致OOM/swap, 回测时间从12s暴增至数分钟
   - 规则: 同一时间只允许1个回测进程

5. **cron与手动优化的冲突**
   - cron的discard流程会删除strategies/中的策略文件
   - 手动优化期间必须暂停cron, 否则策略源码被删
   - 优化完成后恢复cron

6. **动态仓位max(1, ...)下限的必要性**
   - 无下限: position可为0, 需额外空仓判断逻辑
   - 有下限保底1手: buy(price, 0) → vnpy可能报错
   - 最终: return max(1, int(dynamic)), 简单健壮

#### 移植到其他品种
- BTC 1h: Sharpe 1.00, +160.6%, MaxDD -11.8%
- BNB 1h: Sharpe 0.41, +55.3%, MaxDD -17.1%
- BNB参数未优化, 换品种需重优化参数
